...
def docenti_classe(file_orario, classe, file_output):
	...
	Terzo parametro   - accetta in ingresso il percorso del file in cui scrivere l'elaborazione 
    """
...
def orario_docente(file_orario, docente, file_output):
...
def ore_diposizione(file_orario, docente, file_output):
    """
...
def ore_disponibile(file_orario, ora, giorno, file_output):
    """
	...
	Quarto parametro   - accetta in ingresso il percorso del file in cui scrivere l'elaborazione
    """
    ...
    if not os.path.exists(file_orario):
        ...
        while (riga != ""):
            riga = riga.split(SEPARATORE)
            posizione = GIORNI_SET[giorno.capitalize()] + int(ora)
            if (riga[posizione] != "   "):
                elenco_docenti.append(riga[0].strip())
            riga = orario.readline().strip() #Lettura della riga successiva
